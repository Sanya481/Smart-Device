...
var footer = document.querySelector('#footer'); // Блок с аккордионами

var accordionContainer = footer.querySelector('[data-accordion-container]'); // Все аккордионы

var accordions = footer.querySelectorAll('[data-accordion-block]'); // Проверка работоспособности элементов при отключенном JavaScript
...
/* harmony import */ var _modal_callback_js__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_modal_callback_js__WEBPACK_IMPORTED_MODULE_3__);
...
var header = pageBody.querySelector('#header'); // Кнопка открытия модального окна

var orderСallButton = header.querySelector('#header__button'); // Секция с модальным окном для обратной связи

var callbackModalSection = pageBody.querySelector('#callback-modal-section'); // Модальное окно с формой для обратной связи

var callbackModal = pageBody.querySelector('#callback-modal'); // Форма обратной связи

var callbackMmodalForm = pageBody.querySelector('#callback-modal-form'); // if (callbackModal) {
// Кнопка закрытия модального окна

var callbackModalBtnClose = callbackModalSection.querySelector('#callback-modal__btn-close'); // Поле для ввода имени в модальном окне

var userNameInputOnModal = callbackModalSection.querySelector('[data-input-username]'); // Поле для ввода телефона в модальном окне

var userPhoneInputOnModal = callbackModalSection.querySelector('[data-input-phone]'); // Поле для ввода вопроса в модальном окне

var questionTextareaOnModal = callbackModalSection.querySelector('[data-textarea-question]'); // Проверяем есть ли поддержка хранилища
...
var about = document.querySelector('#about'); // Кнопка - больше информации

var aboutButton = about.querySelector('#about__button'); // Блок с информацией

var aboutInfo = about.querySelector('#about__info'); // Элемент с дополнительной информацией

var aboutMoreInfo = about.querySelector('#about__more-info'); // Проверка работоспособности элементов при отключенном JavaScript
...
var phoneInputs = Array.from(document.querySelectorAll('input[data-tel-input]'));
...
var isBackspaceKey = function isBackspaceKey(evt) {
...
var onPhoneInput = function onPhoneInput(evt) {
  var input = evt.target; // Положение курсора

  var selectionStart = input.selectionStart;
  var inputNumbersValue = getInputNumbersValue(input);
  var formattedInputValue = ''; // Если после обрезки символов цифр не осталось - оставляем пустую строку
...
var onPhoneKeyDown = function onPhoneKeyDown(evt) {
  var inputValue = evt.target.value.replace(regExp, '');
