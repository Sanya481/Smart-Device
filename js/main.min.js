...
// Блок с аккордионами
var accordionContainer = document.querySelector('[data-accordion-container]'); // Все аккордионы

var accordions = document.querySelectorAll('[data-accordion-block]'); // Проверка работоспособности элементов при отключенном JavaScript

var checkWorkJs = function checkWorkJs() {
...
var onOpenNavList = function onOpenNavList(evt) {
  if (evt.target.matches('H3')) {
    var accordionBlock = evt.target.closest('[data-accordion-block]');
    var accordionItems = accordionBlock.querySelector('[data-accordion-items]');
    var accordionTitle = accordionBlock.querySelector('[data-accordion-title]');
...
/***/ "./js/intro-callback-btn-hover.js":
/*!****************************************!*\
  !*** ./js/intro-callback-btn-hover.js ***!
  \****************************************/
/*! exports provided: checkMouseMove */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "checkMouseMove", function() { return checkMouseMove; });
var breakpoint = window.matchMedia('(min-width:1024px)'); // Ссылка для перехода к секции обратного вызова

var introCallbackBtn = document.querySelector('[data-intro-callback-btn]'); // Функция отлавливает положение курсора в ссылке

var checkMouseMove = function checkMouseMove() {
  if (breakpoint.matches || introCallbackBtn) {
    introCallbackBtn.onmousemove = function (evt) {
      var x = evt.pageX - evt.target.offsetLeft;
      var y = evt.pageY - evt.target.offsetTop;
      evt.target.style.setProperty('--x', "".concat(x, "px"));
      evt.target.style.setProperty('--y', "".concat(y, "px"));
    };
  }
};
...
/* harmony import */ var _more_information_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./more-information.js */ "./js/more-information.js");
/* harmony import */ var _more_information_js__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_more_information_js__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _modal_callback_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modal-callback.js */ "./js/modal-callback.js");
/* harmony import */ var _footer_accordion_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./footer-accordion.js */ "./js/footer-accordion.js");
/* harmony import */ var _footer_accordion_js__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_footer_accordion_js__WEBPACK_IMPORTED_MODULE_4__);
/* harmony import */ var _intro_callback_btn_hover_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./intro-callback-btn-hover.js */ "./js/intro-callback-btn-hover.js");
/* harmony import */ var _phone_input_mask_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./phone-input-mask.js */ "./js/phone-input-mask.js");
...
window.addEventListener('DOMContentLoaded', function () {
  // Utils
  // ---------------------------------
  Object(_intro_callback_btn_hover_js__WEBPACK_IMPORTED_MODULE_5__["checkMouseMove"])();
  Object(_utils_ios_vh_fix__WEBPACK_IMPORTED_MODULE_0__["iosVhFix"])(); // Находим все поля ввода телфона и вешаем обработчик
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      for (var _i = 0, _arr = ['input', 'blur', 'focus']; _i < _arr.length; _i++) {
        var ev = _arr[_i];
        elem.addEventListener(ev, _phone_input_mask_js__WEBPACK_IMPORTED_MODULE_6__["onEventCalllback"]);
      }
    } // Modules
    // ---------------------------------
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _trap_focus_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./trap-focus.js */ "./js/trap-focus.js");

var pageBody = document.body; // Секция в которой находится нужный нам элемент для удобства поиска

var header = document.querySelector('#header'); // Кнопка открытия модального окна

var orderСallButton = document.querySelector('#header__button'); // Секция с модальным окном для обратной связи

var callbackModalSection = document.querySelector('#callback-modal-section'); // Модальное окно с формой для обратной связи

var callbackModal = document.querySelector('#callback-modal'); // Форма обратной связи

var callbackMmodalForm = document.querySelector('#callback-modal-form');
...
if (callbackModal) {
  ...
  var closeModalCallback = function closeModalCallback() {
    ...
    callbackModalSection.classList.remove('show'); // Удалили обработчик на закрытие по крестику

    callbackModalBtnClose.removeEventListener('click', onModaCallbackClose); // Удалили обработчик на закрытие по нажатию на клавишу Escape

    document.removeEventListener('keydown', onModalCallbackEsc); // Удалили обработчик на закрытие по клику вне модального окна

    document.removeEventListener('click', onClickOverlay); // Удалили обработчик проверерки заполненности полей ввода для имени и телефона

    callbackMmodalForm.removeEventListener('submit', checkFillInputField); // Удалили обработчик на ловушку фокуса

    callbackModal.removeEventListener('keydown', _trap_focus_js__WEBPACK_IMPORTED_MODULE_0__["trapFocus"]);
    orderСallButton.focus();
  };
  /* Функция закрытия модального окна при клике вне области */


  var onClickOverlay = function onClickOverlay(evt) {
    var elementsСlickArea = !evt.composedPath().includes(callbackModal);
  ...
  var onModaCallbackClose = function onModaCallbackClose() {
    closeModalCallback();
  };
  /* Функция закрытия модального окна по нажатию на Escape */


  var onModalCallbackEsc = function onModalCallbackEsc(evt) {
    if (isEscapeKey(evt)) {
      evt.preventDefault();
      closeModalCallback();
    }
  ...
  var checkFillInputField = function checkFillInputField(evt) {
  ...
  var callbackModalBtnClose = document.querySelector('#callback-modal__btn-close'); // Поле для ввода имени в модальном окне

  var userNameInputOnModal = document.querySelector('[data-input-username]'); // Поле для ввода телефона в модальном окне

  var userPhoneInputOnModal = document.querySelector('[data-input-phone]'); // Поле для ввода вопроса в модальном окне

  var questionTextareaOnModal = document.querySelector('[data-textarea-question]'); // Проверяем есть ли поддержка хранилища

  var isStorageSupport = true;
  var userNameStorage = '';
  var userPhoneStorage = '';
  ...
  var onModalCallbackOpen = function onModalCallbackOpen(evt) {
    evt.preventDefault();
    evt.stopPropagation();
    ...
    callbackModalBtnClose.addEventListener('click', onModaCallbackClose); // Добавили обработчик на закрытие по нажатию на клавишу Escape

    document.addEventListener('keydown', onModalCallbackEsc); // Добавили обработчик на закрытие по клику вне модального окна

    document.addEventListener('click', onClickOverlay); // Добавили обработчик проверерки заполненности полей ввода для имени и телефона

    callbackMmodalForm.addEventListener('submit', checkFillInputField); // Добавили обработчик на ловушку фокуса

    callbackModal.addEventListener('keydown', _trap_focus_js__WEBPACK_IMPORTED_MODULE_0__["trapFocus"]);
  };
...
var aboutButton = document.querySelector('#about__button'); // Блок с информацией

var aboutInfo = document.querySelector('#about__info'); // Элемент с дополнительной информацией

var aboutMoreInfo = document.querySelector('#about__more-info'); // Проверка работоспособности элементов при отключенном JavaScript
...
/***/ "./js/phone-input-mask.js":
/*!********************************!*\
  !*** ./js/phone-input-mask.js ***!
  \********************************/
/*! exports provided: onEventCalllback */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "onEventCalllback", function() { return onEventCalllback; });
// https://webkaa.ru/javascript/maska-nomera-telefona-js
var onEventCalllback = function onEventCalllback(evt) {
  var el = evt.target;
...
var focusableEls = callbackModal.querySelectorAll('a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="tel"]:not([disabled]), input[type="text"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled])');
